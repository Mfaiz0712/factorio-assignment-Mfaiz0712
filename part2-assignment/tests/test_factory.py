...
def test_sample(tmp_path):
    exe = ["python", str(Path(__file__).parent.parent / "factory" / "main.py")]
    proc = subprocess.run(exe, input=SAMPLE_IN.encode(), capture_output=True, timeout=5)
    assert proc.returncode == 0, proc.stderr.decode()
    out = json.loads(proc.stdout)
    assert out["status"] == "ok"
    assert abs(out["per_recipe_crafts_per_min"]["green_circuit"] - 1800) < 1e-4
